...
/**
 * @file   main_bm3d_LF.cpp
 * @brief  Main executable file to apply BM3D to each SAI of the light field.
 *
 ...
 * - Angular size will be denoted awidth and aheight (and associated derivations) for angular width and angular height;
...
int main(int argc, char **argv)
{
    ...
    char *LF_input_name, *LF_noisy_name, *LF_basic_name, *LF_denoised_name, *LF_diff_name;
    bool gt_exists;
    unsigned awidth, aheight, anHard, anWien, ang_major;
    float fSigma, lambdaHard3D;
    unsigned NHard, NWien;
    unsigned nHard, nWien;
    unsigned kHard, kWien, pHard, pWien;
    unsigned tau_2D_hard, tau_2D_wien;
    bool useSD_1, useSD_2;
    unsigned color_space;
    unsigned nb_threads;
    char* psnr_file_name;

    if(get_params_BM3D(argc, argv, &LF_input_name, gt_exists, awidth, aheight, anHard, anWien, ang_major, fSigma, lambdaHard3D, &LF_noisy_name, &LF_basic_name, &LF_denoised_name, &LF_diff_name,
    ...
    if(gt_exists)
    {
        ...
        if(load_LF(LF_input_name, LF, LF_SAI_mask, awidth, aheight, &width, &height, &chnls) != EXIT_SUCCESS)
            return EXIT_FAILURE;
    ...
    if(gt_exists)
    {
        ...
        if (save_LF(LF_noisy_name, "SAI", LF_noisy, LF_SAI_mask, ang_major, awidth, aheight, width, height, chnls) != EXIT_SUCCESS)
            return EXIT_FAILURE;
    ...
    else //! Read noisy light field
    {
        ...
        if(load_LF(LF_noisy_name, LF_noisy, LF_SAI_mask, awidth, aheight, &width, &height, &chnls) != EXIT_SUCCESS)
            return EXIT_FAILURE;
    ...
    //* Denoising
    //* -------------------------------------------------------------------------------------------------------------------------------------------------------------------
    cout << endl << " ---> Running LFBM3D filter <--- " << endl << endl;
    timestamp_t start_bm3d = get_timestamp();

    if (run_bm3d_LF(fSigma, LF_noisy, LF_SAI_mask, LF_basic, LF_denoised, width, height, chnls,
                nHard, nWien, kHard, kWien, NHard, NWien, pHard, pWien,
                useSD_1, useSD_2, tau_2D_hard, tau_2D_wien, lambdaHard3D, color_space, nb_threads, "SAI") != EXIT_SUCCESS)
        return EXIT_FAILURE;
    ...
    timestamp_t start_save = get_timestamp();
	if (save_LF(LF_basic_name, "SAI", LF_basic, LF_SAI_mask, ang_major, awidth, aheight, width, height, chnls) != EXIT_SUCCESS)
        return EXIT_FAILURE;
    timestamp_t end_save = get_timestamp();
    float save_elapsed_secs = float(end_save-start_save) / 1000000.0f;
    cout << "done in " << save_elapsed_secs << "s." << endl;
	...
	if (save_LF(LF_denoised_name, "SAI", LF_denoised, LF_SAI_mask, ang_major, awidth, aheight, width, height, chnls) != EXIT_SUCCESS)
        return EXIT_FAILURE;
    ...
    if(gt_exists)
    {
        ...
        start_save = get_timestamp();
        if (save_LF(LF_diff_name, "SAI", LF_diff, LF_SAI_mask, ang_major, awidth, aheight, width, height, chnls) != EXIT_SUCCESS)
            return EXIT_FAILURE;
        end_save = get_timestamp();
        save_elapsed_secs = float(end_save-start_save) / 1000000.0f;
    ...
    timestamp_t end_bm3d = get_timestamp();
    float bm3d_elapsed_secs = float(end_bm3d-start_bm3d) / 1000000.0f;
    cout << "Total elapsed time = " << bm3d_elapsed_secs << "s." << endl;
