...
,   const unsigned nb_threads
    ...
    fftwf_plan plan_2d_for_3[nb_threads];
    fftwf_plan plan_2d_inv[nb_threads];
    fftwf_plan plan_4d[nb_threads];
    fftwf_plan plan_4d_inv[nb_threads];
    fftwf_plan plan_4d_sa[nb_threads][asize_sw-1];     //! Assign a 1d transform for each possible size between 2 and angular patch size
    fftwf_plan plan_4d_sa_inv[nb_threads][asize_sw-1];
    fftwf_plan plan_5d[nb_threads];
    fftwf_plan plan_5d_inv[nb_threads];
    ...
    if (nb_threads == 1)
    {
        ...
        while(countProcSAI) //! Loop on non-processed SAI
        {
            ...
            #pragma omp parallel for
            for(unsigned s_asw = 0; s_asw < asize_sw; s_asw++)
            {
            ...
            while(countProcSAI_asw)
            {
                ...
                else //! Process the SAI with the most pixels to denoised
                {
                    int countNoisyPix = -1;
                    #pragma omp parallel for
                    for(unsigned st = 0; st < (asize_sw * asize_sw); st++)
                    {
                        if(procSAI_asw[st] == 0)
                        {
