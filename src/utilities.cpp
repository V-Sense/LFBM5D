...
,   const float sigma
    ...
    // The line below creates the same noise for several SAI (because time is in seconds) - MA
    //mt_init_genrand((unsigned long int) time (NULL) + (unsigned long int) getpid());
    #ifndef WINDOWS
        struct timeval tp;
        gettimeofday(&tp, NULL);
        unsigned long int mstime = tp.tv_sec * 1000 + tp.tv_usec / 1000;
        mt_init_genrand(mstime + (unsigned long int) getpid());
    #else
    	clock_t time = clock();
    	double duration = time / (double)CLOCKS_PER_SEC;
    	unsigned long int mstime = (long)(duration*1000)%1000;
    	mt_init_genrand(mstime + (unsigned long int) GetCurrentProcessId());
    #endif
...
/**
 * @brief Remove boundaries by symetry
 *
 * @param img : will contain image without boundaries
 * @param img_sym : img with symetrized boundaries
 * @param width, height, chnls : size of img
 * @param N : size of the boundary
 *
 * @return none.
...
,   const unsigned N
    ...
    if (img.size() != width * height * chnls)
        img.resize(width * height * chnls);
    ...
    for (unsigned c = 0; c < chnls; c++)
    {
        const unsigned dc_b = c * w_b * h_b + N * w_b + N;
        unsigned dc = c * width * height;
        for (unsigned i = 0; i < height; i++)
            for (unsigned j = 0; j < width; j++, dc++)
                img[dc] = img_sym[dc_b + i * w_b + j];
    }
}
...
,   const float sigma
    ...
    for (unsigned k = 0; k < size; k++)
        {
            float value = s > 0.0 ? (img_1[k] - img_2[k] + s) * 255.0f / (2.0f * s) : fabs(img_1[k] - img_2[k]); // * 255.0f / 2.0f;
 ...
 *
 * @param plan: fftwf_plan to allocate;
 * @param N1, N2: size of the patch to apply the 2D transform;
 * @param kind: forward or backward;
 * @param nb: number of 2D transform which will be processed.
 *
 * @return none.
...
,   const unsigned nb
    ...
    int            nb_table[2]   = {N1, N2};
    int            nembed[2]     = {N1, N2};
    fftwf_r2r_kind kind_table[2] = {kind, kind};

    float* vec = (float*) fftwf_malloc(N1 * N2 * nb * sizeof(float));
    (*plan) = fftwf_plan_many_r2r(2, nb_table, nb, vec, nembed, 1, N1 * N2, vec,
                                  nembed, 1, N1 * N2, kind_table, FFTW_ESTIMATE);
...
,   const unsigned nb
    ...
    int nb_table = N;
    int nembed  = N * nb;
    fftwf_r2r_kind kind_table = kind;
