...
/**
 * @brief Apply forward Haar wavelet transform on vec
 *
 * @param vec: vector on which a Haar transform will be applied.
 *        It will contain the transform at the end;
 * @param tmp: must have the same size as vec. Used for convenience;
 * @param N, D: the Haar transform will be applied on vec[D] -> vec[D + N].
 *        N must be a power of 2!!!!
 *
 * @return None.
 **/
...
,   const unsigned D
    ...
    else if (N == 2)
    {
        const float a = vec[D + 0];
        const float b = vec[D + 1];
        vec[D + 0] = (a + b) * (float)(SQRT2_INV);
        vec[D + 1] = (a - b) * (float)(SQRT2_INV);
    }
    else
    {
        const unsigned n = N / 2;
        for (unsigned k = 0; k < n; k++)
        {
            const float a = vec[D + 2 * k];
            const float b = vec[D + 2 * k + 1];
            tmp[k]     = (a + b) * (float)(SQRT2_INV);
            tmp[n + k] = (a - b) * (float)(SQRT2_INV);
        }
        for (unsigned k = 0; k < N; k++)
            vec[D + k] = tmp[k];
        
        haar_forward(vec, tmp, n, D);
    }
}


/**
 * @brief Apply inverse Haar wavelet transform on vec
 *
 * @param vec: vector on which a inverse Haar transform will be applied;
 * @param tmp: must have the same size as vec. Used for convenience;
 * @param n: recursive size of the inverse transform;
 * @param N, D: the Haar inverse transform will be applied on vec[D] -> vec[D + N].
 *        N must be a power of 2!!!!
 *
 * @return None.
 **/
...
,   const unsigned D
    ...
    else
    {
        for (unsigned k = 0; k < n; k++)
        {
            const float a = vec[D + k];
            const float b = vec[D + n + k];
            tmp[2 * k]     = (a + b) * (float)(SQRT2_INV);
            tmp[2 * k + 1] = (a - b) * (float)(SQRT2_INV);
        }
        n *= 2;
        for (unsigned k = 0; k < n; k++)
            vec[D + k] = tmp[k];
        
        haar_inverse(vec, tmp, n, N, D);
    }
}
